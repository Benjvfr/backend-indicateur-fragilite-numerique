...
# Part des ménages d’une personne / région
newdf['Part des ménages d’une personne / région'] = np.nan

for i, r in (df.groupby(df['REG'])['C16_MENPSEUL'].sum()*100/df.groupby(df['REG'])['C16_MEN'].sum()).to_frame('Moyenne région').iterrows():
    newdf['Part des ménages d’une personne / région'].loc[df['REG'] == i] = r['Moyenne région']

# Points de la part des ménages d’une personne / commune
newdf['Points de la part des ménages d’une personne / commune'] = (((newdf['Part des ménages d’une personne / commune']-newdf['Part des ménages d’une personne / région'])/newdf['Part des ménages d’une personne / région'])+1)*100
...
# Part des familles monoparentales / région
newdf['Part des familles monoparentales / région'] = np.nan

for i, r in (df.groupby(df['REG'])['C16_FAMMONO'].sum()*100/df.groupby(df['REG'])['C16_MEN'].sum()).to_frame('Moyenne région').iterrows():
    newdf['Part des familles monoparentales / région'].loc[df['REG'] == i] = r['Moyenne région']

# Points de la part des familles monoparentales / commune
newdf['Points de la part des familles monoparentales / commune'] = (((newdf['Part des familles monoparentales / commune']-newdf['Part des familles monoparentales / région'])/newdf['Part des familles monoparentales / région'])+1)*100
